...
s|[[BackendForFrontend]]BFF
| Backend for Frontend
| A server providing an API that is called by browser
...
s| Domain
| (Fach-) Domäne
| A domain is a field of knowledge, impact or activity. The area of expertise, for which a user uses an application, is the domain of the software. See also <<tier_layer_domain>>
...
s|IdP
|
|Identity Provider. A server, that implements OIDC ({idp})
...
s|[[OAUTH]]OAuth 2
|
|Open Authorization, Name of the Specification (Version 2)

s|OIDC
|
|Open ID Connect, Name of the Specification
...
s| Tier
| Stufe
| The term tier refers to physical entities like deliverables or deployable applications. See also figure <<tier_layer_domain>>

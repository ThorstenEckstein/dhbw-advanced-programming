...
Nutzt die SQL Statements aus der Datei
 ...
 course-7/dbunit/demonstration.sql

um diese einfach selbst einmal gegen die lokale bzw. eigene H2 Datenbank auszuführen. Die Befehle können einfach aus der Datei in die geöffnete/gestartete _Datenbank Console_ kopiert werden, dann erscheinen in den Zeilen jeweils _execution_ Symbole: image:images/execute-stmt-icon.png[width="25%"] ...


Übung 2::
Nutzt die SQL Statements aus der Datei

 course-7/dbunit/join-example.sql

um diese einfach selbst einmal gegen die lokale bzw. eigene H2 Datenbank auszuführen.
...
[subs=attributes]
 /test/de/dhbw/course7/exercises/ExerciseTests.java

...
Erstelle ein Interface für einen Taschenrechner, der die vier Grundrechenarten in Form von Methoden zur Verfügung stellt. Der Taschenrechner sollte mit einem beliebigen Zahlentyp umgehen können.

TIP: Zahlentypen in Java haben eine gemeinsame Superklasse `java.lang.Number`.

_Optional: Realisiere auch einen konkreten Taschenrechner, der das Interface implementiert und schreibe einen kleinen Test, der die Funktionsfähigkeit mindestens einer der Rechenarten am Beispiel testet._

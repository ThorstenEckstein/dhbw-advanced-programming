...
* Einheitliche Schnittstelle (konventionelle Ressourcen-URLs)
* Es werden die Standard HTTP Operation genutzt (`GET, PUT, POST, DELETE, ...`)
...
public class SystemDataController extends AbstractController {
    ...
    protected void assertAlreadyPersistent(String id) {
        if (id == null) {
            ExceptionContext error = ExceptionContextBuilder.error(
                    "Updating 'System' with name '%s' failed, because ID is missing!",id);
    ...
    private void assertNotAlreadyPersistent(String representationId) {
        ...
        if (persistentSystem != null) {
            ExceptionContext error = ExceptionContextBuilder.error(
                    "Unable to create system, because a system with ID '%s' (and name '%s') already exists!",
...
public class Consumer {
    ...
    public void consumeMessage(
        ...
        final Train verifiedTrain = verifiy(train);

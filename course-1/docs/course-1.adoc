...
TIP: Richtet euch bitte einen lokalen _Workspace_ ein, d.h. ein eigenes Wurzelverzeichnis, in das das SourceCode-Repository bzw. die Kursinhalte "gespeichert" werden kann.

Natürlich kann man hier den persönlichen Präferenzen für dieses Wurzelverzeichnis folgen ...

Zum Beispiel für Windows:

 C:\Projekte\ProgrammierungJavaII

oder unter MacOS:

 /Users/<user>/Projects/ProgrammierungJavaII

Ich persönlich richte meine lokale Arbeitsumgebung (den _Workspace_) wann immer möglich, etwa für ein Projekt, mit exakt der gleichen Struktur wie die des (Online/Remote-) Repositories ein.
...
Bitte zuerst prüfen, ob `git` korrekt installiert ist. Das geht am besten in einem Terminal durch
...
Zur Prüfung, auf welchem Branch man sich aktuell befindet, wechselt man in das neue Verzeichnis und gibt Folgendes ein:

 git branch

Hier sollte jetzt das Folgende erscheinen:
...
[red]#*Git > Clone mit Nutzung von IntelliJ & Screenshots*#
...
- In diesen neuen, noch leeren Ordner wechseln

 cd Projekte

AUSFLUG: JAVA VERSION PRÜFEN

- Prüfen, ob `java` vorhanden ist

 java -version

AUSFLUG: GIT VERSION PRÜFEN

- Prüfen, ob `git` vorhanden ist

 git --version

AUSFLUG: ANZEIGE VON UMGEBUNGSVARIABLEN

...
Verify Arguments::
This is a good practice anyway. Validation input data in methods or algorithms is always recommended. These checks (assertions) can be functional (e.g. checks for _value ranges_) as well as technical (e.g. checks for _null_).
...
Every code shows an _impression_ of its complexity, and the calculable metric can be checked, in rela life projects most often via "Code Metric Tools".

An example:

[source,java, lines]
----
include::{course-8-src}/complexity/OperationMonitor.java[tags="cyclo-complex"]
----
...
See also -> `Course 9` (Strategy Pattern)
